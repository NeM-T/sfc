...
let isWhite c =
  ...
  (||)
    ((||)
      (Nat.eqb n0 ((fun x -> x + 1) ((fun x -> x + 1)
        ((fun x -> x + 1) ((fun x -> x + 1)
        ((fun x -> x + 1) ((fun x -> x + 1)
        ((fun x -> x + 1) ((fun x -> x + 1)
        ((fun x -> x + 1) ((fun x -> x + 1)
...
let isLowerAlpha c =
  ...
  (&&)
    (Nat.leb ((fun x -> x + 1) ((fun x -> x + 1)
      ...
      0)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
      n0)
    (Nat.leb n0 ((fun x -> x + 1) ((fun x -> x + 1)
      ((fun x -> x + 1) ((fun x -> x + 1)
      ((fun x -> x + 1) ((fun x -> x + 1)
      ((fun x -> x + 1) ((fun x -> x + 1)
      ((fun x -> x + 1) ((fun x -> x + 1)
...
let classifyChar c =
  ...
  then White
       ...
       then Alpha
       else if isDigit c then Digit else Other
...
let rec tokenize_helper cls acc xs =
 ...
 (fun f c ->
  ...
  f (h 0) (h 1) (h 2) (h 3) (h 4) (h 5) (h 6) (h 7))
             ...
             if b
             then if b0
                  then tokenize_helper Alpha (x :: acc)
                         xs'
                  else if b1
 ...
 (fun f c ->
  ...
  f (h 0) (h 1) (h 2) (h 3) (h 4) (h 5) (h 6) (h 7))
             ...
             if b
             then if b0
                  then tokenize_helper Digit (x :: acc)
                         xs'
                  else if b1
 ...
 (fun f c ->
  ...
  f (h 0) (h 1) (h 2) (h 3) (h 4) (h 5) (h 6) (h 7))
             ...
             if b
             then if b0
                  then tokenize_helper Other (x :: acc)
                         xs'
                  else if b1
...
type 't parser0 =
  token list -> ('t * token list) optionE

(** val many_helper :
    'a1 parser0 -> 'a1 list -> int -> token list ->
    ('a1 list * token list) optionE **)
...
(** val many :
    'a1 parser0 -> int -> 'a1 list parser0 **)

let rec many p steps =
  many_helper p [] steps

(** val firstExpect :
    token -> 'a1 parser0 -> 'a1 parser0 **)
...
(** val parseIdentifier :
    token list -> (char list * token list) optionE **)
...
(** val parseNumber :
    token list -> (int * token list) optionE **)
...
| x :: xs' ->
  ...
  then SomeE
         ((fold_left (fun n0 d ->
            ...
            (list_of_string x) 0), xs')
  else NoneE
         ('E'::('x'::('p'::('e'::('c'::('t'::('e'::('d'::(' '::('n'::('u'::('m'::('b'::('e'::('r'::[])))))))))))))))

(** val parsePrimaryExp :
    int -> token list -> (aexp * token list) optionE **)
...
let rec parsePrimaryExp steps xs =
  (fun zero succ n -> if n=0 then zero () else succ (n-1))
    ...
    | SomeE x ->
      let (i, rest) = x in SomeE ((AId i), rest)
    | NoneE _ ->
      (match parseNumber xs with
       | SomeE x ->
         let (n0, rest) = x in SomeE ((ANum n0), rest)
       | NoneE _ ->
         (match firstExpect ('('::[])
                  (parseSumExp steps') xs with
          | SomeE x ->
            let (e, rest) = x in
            (match expect (')'::[]) rest with
             | SomeE x0 ->
               let (_, rest') = x0 in SomeE (e, rest')
             | NoneE err -> NoneE err)
          | NoneE err -> NoneE err)))
    steps
    ...
    | SomeE x ->
      ...
      (match many
       ...
       | SomeE x0 ->
         let (es, rest') = x0 in
         SomeE
         ((fold_left (fun x1 x2 -> AMult (x1, x2)) es e),
         rest')
       | NoneE err -> NoneE err)
    | NoneE err -> NoneE err)
    steps
...
let rec parseAtomicExp steps xs =
  (fun zero succ n -> if n=0 then zero () else succ (n-1))
    ...
    | SomeE x ->
      let (_, rest) = x in SomeE (BTrue, rest)
    | NoneE _ ->
      (match expect
               ('f'::('a'::('l'::('s'::('e'::[]))))) xs with
       | SomeE x ->
         let (_, rest) = x in SomeE (BFalse, rest)
       | NoneE _ ->
         (match firstExpect ('~'::[])
                  (parseAtomicExp steps') xs with
          | SomeE x ->
            let (e, rest) = x in SomeE ((BNot e), rest)
          | NoneE _ ->
            (match firstExpect ('('::[])
    ...
    | SomeE x ->
      ...
      (match many
       ...
       | SomeE x0 ->
         let (es, rest') = x0 in
         SomeE
         ((fold_left (fun x1 x2 -> BAnd (x1, x2)) es e),
         rest')
       | NoneE err -> NoneE err)
    | NoneE err -> NoneE err)
    steps
...
let rec parseSimpleCommand steps xs =
  (fun zero succ n -> if n=0 then zero () else succ (n-1))
    ...
    | SomeE x ->
      let (_, rest) = x in SomeE (CSkip, rest)
    | NoneE _ ->
      (match firstExpect ('T'::('E'::('S'::('T'::[]))))
